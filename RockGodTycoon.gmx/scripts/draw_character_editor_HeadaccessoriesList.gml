/// draw_character_editor_HeadaccessoriesList()
...
if ( ds_list_size(global.item_list_headaccessories) > 8 ){
...
var i = 0;var r = 0;var size = ds_list_size(global.item_list_headaccessories);var size2 = 0;
...
equiplist_inst[|size2].Item_Name = global.item_list_headaccessories[|size2];
equiplist_inst[|size2].Item_Price = global.price_list_headaccessories[|size2];
}
}

with(obj_equiplist_button){
var math_y = y-(((size-8.85)*80)*list_index_start);
if ( Item_Name != "-Head Bands" && Item_Name != "-Glasses" && Item_Name != "-Piercings" ){
...
if ( mouse_check_button_pressed(mb_left) ){
if ( !Locked ){
scr_equip_item(obj_character_pose1,"Head Accessory",Item_Name);}}}}
}
else{
if ( math_y > 85 && math_y < 865 ){
draw_external_sprite(global.sprite_map[?"Character Creation\Equip List Sub Menu\Category Tag Piece.png"],0,311,math_y);
}}
}
